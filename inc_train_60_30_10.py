...
csv_folder = "/home/bbboming/GDrive/Yonsei/논문/APSIPA/start_2/60_30_10excel"
...
checkpoint = '/home/bbboming/HDD/Paper/inc_with_bay/pytorch-cifar100_60_40/checkpoint/resnet50/2020-10-09T16:50:52.721267/resnet50-129-best.pth'

class layer_sharing_acc():
    def __init__(self, acc_0=1.0, acc_53=0.0, inc_index=1):
        self.basenet = resnet50(60).cuda()
    ...
    def f(self,x, return_acc=False): #x, layer number to calculate
        ...
        checkpoint_path = os.path.join(settings.CHECKPOINT_PATH, 'resnet50_inc%d'%self.inc_index, settings.TIME_NOW)
        ...
        for epoch in range(1, settings.EPOCH):
            ...
            print('Test set: Average loss: {:.4f}, Accuracy: {:.4f} (BaseAcc {:.4f} IncAcc {:.4f})'.format(avg_loss,acc, base_acc, inc_acc))
...
def estimate_sharing_layer_count(acc_0=1.0, acc_53=0.0, inc_index=1):
    func = layer_sharing_acc(acc_0, acc_53, inc_index)

    space = [{'name': 'var_1', 'type': 'discrete','domain': range(1,54)},
             {'name': 'var_2', 'type': 'continuous','domain': (0,1)}]
    space2 = [{'name': 'var_1', 'type': 'discrete','domain': range(25,54)}]
...
if __name__ == "__main__":
    inc_index=2
    obj = layer_sharing_acc(inc_index=inc_index)
    acc_0 = obj.f(np.array([0]), return_acc=True)
    acc_53 = obj.f(np.array([53]), return_acc=True)
    print(estimate_sharing_layer_count(acc_0, acc_53, inc_index))
